...
- (void) validateText
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ( [validationInfo objectForKey:VALIDATION_INDICATOR_COLOR] ) {
            self->_validationYesColor = [validationInfo objectForKey:VALIDATION_INDICATOR_COLOR];
        }
        if ( [validationInfo objectForKey:VALIDATION_INDICATOR_COLOR] ) {
            self->_validationNoColor = [validationInfo objectForKey:VALIDATION_INDICATOR_COLOR];
        }
